...
export default function Timetable({ onBack }: { onBack: () => void }) {
  ...
  // Mode: 'class' (جدول فصل) or 'teacher' (جدول معلم)
  const [viewMode, setViewMode] = useState<'class' | 'teacher'>('class');

  // Class Selection State
  const grades = Object.keys(gradeFees);
  const [selectedGrade, setSelectedGrade] = useState(grades[0] || 'الصف الأول');
  ...
  // Auto select classroom when grade changes
  useMemo(() => {
  ...
  // Find all schedule entries for a specific teacher across all classes
  const getTeacherSchedule = (teacherName: string) => {
    const teacherEntries: { day: string; periodId: number; subject: string; classKey: string }[] = [];
    if (!teacherName) return teacherEntries;
  ...
  const logoHtml = schoolLogo?.startsWith('data:image') 
    ? `<img src="${schoolLogo}" style="height:50px;" />` 
  ...
  const handlePrintClassTimetable = () => {
    ...
    const rows = PERIODS.map(p => {
      ...
      const cells = DAYS.map(d => {
        ...
        return `<td style="border:1px solid ${c.border};padding:3px;text-align:center;background:${c.bg};"><div style="font-weight:bold;font-size:11px;color:${c.text};">${entry.subject}</div>${entry.teacher ? `<div style="font-size:9px;color:#555;">${entry.teacher}</div>` : ''}</td>`;
      }).join('');
  ...
  const handlePrintTeacherTimetable = () => {
    ...
    const teacherSchedule = getTeacherSchedule(selectedTeacherName);

    const headerCells = DAYS.map(d => `<th style="background:#065f46;color:#fff;padding:8px 4px;border:1px solid #064e3b;font-size:12px;">${d}</th>`).join('');
    
    const rows = PERIODS.map(p => {
      if (p.isBreak) return `<tr><td colspan="${DAYS.length + 2}" style="background:#f59e0b;color:#fff;text-align:center;padding:5px;font-weight:bold;font-size:11px;">☕ استراحة (${p.time})</td></tr>`;
      
      const cells = DAYS.map(d => {
        const match = teacherSchedule.find(s => s.day === d && s.periodId === p.id);
        if (!match) return `<td style="border:1px solid #d1d5db;padding:4px;text-align:center;color:#ccc;font-size:10px;">-</td>`;
        const c = SUBJECT_COLORS[match.subject] || { bg: '#d1fae5', text: '#065f46', border: '#6ee7b7' };
        return `<td style="border:1px solid ${c.border};padding:4px;text-align:center;background:${c.bg};"><div style="font-weight:bold;font-size:11px;color:${c.text};">${match.subject}</div><div style="font-size:9px;color:#0369a1;font-weight:bold;">${match.classKey}</div></td>`;
      }).join('');

      return `<tr><td style="border:1px solid #d1d5db;padding:3px;text-align:center;font-size:10px;font-weight:bold;background:#f8fafc;">${p.label}</td><td style="border:1px solid #d1d5db;padding:3px;text-align:center;font-size:9px;color:#666;background:#f8fafc;">${p.time}</td>${cells}</tr>`;
    }).join('');

    pw.document.write(`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>جدول المعلم - ${selectedTeacherName}</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;700&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Cairo',sans-serif;direction:rtl;padding:15px;}@page{size:A4 landscape;margin:8mm;}table{width:100%;border-collapse:collapse;}</style></head><body><div style="display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #065f46;padding-bottom:8px;margin-bottom:10px;"><div><h2 style="font-size:16px;">${schoolName}</h2><h3 style="color:#059669;font-size:14px;">جدول حصص المعلم/ة: ${selectedTeacherName} ${teacherObj ? `(${teacherObj.subject})` : ''} - إجمالي الحصص: ${teacherSchedule.length} حصة</h3></div><div>${logoHtml}</div></div><table><thead><tr><th style="background:#065f46;color:#fff;padding:8px 4px;border:1px solid #064e3b;font-size:12px;width:8%;">الحصة</th><th style="background:#065f46;color:#fff;padding:8px 4px;border:1px solid #064e3b;font-size:12px;width:9%;">الوقت</th>${headerCells}</tr></thead><tbody>${rows}</tbody></table></body></html>`);
    pw.document.close(); pw.focus();
    setTimeout(() => pw.print(), 500);
  };
  ...
  // Schedule for selected teacher
  const selectedTeacherSchedule = useMemo(() => {
    return getTeacherSchedule(selectedTeacherName);
  }, [selectedTeacherName, timetables]);
  ...
  return (
    <div style={{ direction: 'rtl', padding: 24, backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', minHeight: '100vh', fontFamily: 'Cairo, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 24 }}>
        <button onClick={onBack} style={{ background: 'linear-gradient(135deg, var(--primary-color), #1e40af)', border: 'none', color: '#ffffff', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', gap: 10, padding: '10px 24px', borderRadius: 30, fontWeight: 'bold', fontFamily: 'Cairo, sans-serif', boxShadow: '0 4px 15px rgba(37, 99, 235, 0.4)' }}>
        ...
        <div style={{ display: 'flex', backgroundColor: 'var(--bg-card)', padding: 4, borderRadius: 12, border: '1px solid var(--border-color)' }}>
          <button
            ...
            style={{
              ...
              backgroundColor: viewMode === 'class' ? '#0056b3' : 'transparent',
          ...
          <button
            ...
            style={{
              ...
              fontSize: 14,
              fontFamily: 'Cairo, sans-serif',
              backgroundColor: viewMode === 'teacher' ? '#059669' : 'transparent',
      ...
      {viewMode === 'class' && (
        <div className="card" style={{ borderRadius: 12, padding: 28 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
            <h2 style={{ margin: 0, color: 'var(--primary-color)', fontSize: 24 }}>📅 الجدول الدراسي الأسبوعي للفصل</h2>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <select value={selectedGrade} onChange={e => setSelectedGrade(e.target.value)} style={{ padding: '10px 18px', borderRadius: 8, border: '2px solid var(--primary-color)', fontSize: 15, fontWeight: 'bold', fontFamily: 'Cairo', color: 'var(--primary-color)', background: 'var(--input-bg)' }}>
                {grades.map(g => <option key={g} value={g}>{g}</option>)}
              </select>
              {currentGradeClasses.length > 0 && (
                <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} style={{ padding: '10px 18px', borderRadius: 8, border: '2px solid var(--primary-color)', fontSize: 15, fontWeight: 'bold', fontFamily: 'Cairo', color: 'var(--primary-color)', background: 'var(--input-bg)' }}>
              ...
              <button onClick={handlePrintClassTimetable} style={btnPrimary}>🖨️ طباعة جدول الفصل</button>
            </div>
          </div>
      ...
      {viewMode === 'teacher' && (
        <div className="card" style={{ borderRadius: 12, padding: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
            <h2 style={{ margin: 0, color: '#059669', fontSize: 24 }}>👨‍🏫 الجدول الدراسي للمعلم</h2>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <select
                value={selectedTeacherName}
                onChange={e => setSelectedTeacherName(e.target.value)}
                style={{ padding: '10px 20px', borderRadius: 8, border: '2px solid #059669', fontSize: 15, fontWeight: 'bold', fontFamily: 'Cairo', color: '#059669', background: 'var(--input-bg)', minWidth: 220 }}
              >
                {teachers.map(t => (
                  <option key={t.id} value={t.name}>{t.name} ({t.subject})</option>
                ))}
              </select>
              <button onClick={handlePrintTeacherTimetable} style={{ ...btnPrimary, backgroundColor: '#059669' }}>
                🖨️ طباعة جدول المعلم
              </button>
            </div>
          </div>

          <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', padding: 14, borderRadius: 8, marginBottom: 20, fontSize: 14, color: '#065f46', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            <span>إجمالي الحصص المسجلة للمعلم/ة <strong>{selectedTeacherName}</strong>: <strong>{selectedTeacherSchedule.length} حصة أسبوعياً</strong></span>
            <span>المادة التخصصية: <strong>{teachers.find(t => t.name === selectedTeacherName)?.subject || '-'}</strong></span>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
              <thead>
                <tr>
                  <th style={{ ...thStyle, background: '#065f46' }}>الحصة</th>
                  <th style={{ ...thStyle, background: '#065f46', fontSize: 11 }}>الوقت</th>
                  {DAYS.map(d => <th key={d} style={{ ...thStyle, background: '#065f46' }}>{d}</th>)}
                </tr>
              </thead>
              <tbody>
                {PERIODS.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td style={timeStyle}>{p.time}</td>
                      {DAYS.map(d => {
                        const match = selectedTeacherSchedule.find(s => s.day === d && s.periodId === p.id);
                        const c = match ? (SUBJECT_COLORS[match.subject] || { bg: '#d1fae5', text: '#065f46', border: '#6ee7b7' }) : null;
                        ...
                        return (
                          ...
                          >
                            {match ? (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                <div style={{ fontWeight: 'bold', color: c?.text, fontSize: 14 }}>{match.subject}</div>
                                <div style={{ fontSize: 12, color: '#0284c7', fontWeight: 'bold' }}>{match.classKey}</div>
                              </div>
                            ) : (
                              <div style={{ color: 'var(--border-color)', fontSize: 16 }}>-</div>
                            )}
                          </td>
                        );
                      })}
