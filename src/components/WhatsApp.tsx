...
export default function WhatsApp({ onBack }: { onBack: () => void }) {
  ...
  const sendSingleMessage = async (studentId: number) => {
    ...
    if (!student || !targetPhone) return false;
    ...
    const phone = formatPhoneForWA(targetPhone);
    const logEntry = { name: student.fatherName, phone: targetPhone, time: new Date().toLocaleTimeString('ar-LY'), status: 'جاري الإرسال' as const };
