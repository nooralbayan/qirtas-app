...
export default function Reports({ onBack }: { onBack: () => void }) {
  ...
  const [activeReport, setActiveReport] = useState<'all' | 'class' | 'paid' | 'delayed' | 'parents' | 'specialNeeds' | 'medical' | 'teacherWorkload' | null>(null);
  ...
  } else if (activeReport === 'paid') {
    // قاموا بدفع الأقساط أو جزء منها (مسدد أو جزئي)
    financialFilteredData = reportData.filter(s => s.status === 'مسدد' || s.status === 'جزئي');
  } else if (activeReport === 'delayed') {
    // المتخلفين عن الدفع (متأخر)
    financialFilteredData = reportData.filter(s => s.status === 'متأخر');
  }

  // Helper to get logical grade weight
  const getGradeWeight = (gradeStr: string) => {
    ...
    const index = grades.findIndex(g => gradeStr.includes(g));
    return index !== -1 ? index : 99;
  };

  // Sort financial reports by Class (logical grade) then by Name alphabetically
  financialFilteredData.sort((a, b) => {
    const weightA = getGradeWeight(a.grade);
    const weightB = getGradeWeight(b.grade);
    ...
    if (a.grade !== b.grade) {
      return a.grade.localeCompare(b.grade, 'ar'); // e.g. الصف الأول - أ comes before الصف الأول - ب
    }
  ...
  // Sort data reports by Class then by Name alphabetically
  dataFilteredStudents.sort((a, b) => {
    const classA = a.classRoom ? `${a.grade} - ${a.classRoom}` : a.grade;
    const classB = b.classRoom ? `${b.grade} - ${b.classRoom}` : b.grade;
    
    const weightA = getGradeWeight(classA);
    const weightB = getGradeWeight(classB);
  ...
  const isFinancialReport = activeReport === 'all' || activeReport === 'class' || activeReport === 'paid' || activeReport === 'delayed';
  ...
  const handlePrintReport = () => {
    ...
    else if (activeReport === 'paid') reportTitle = 'تقرير المسددين (كلي وجزئي)';
    else if (activeReport === 'delayed') reportTitle = 'تقرير المتخلفين عن الدفع';
  ...
  const handleExportExcel = () => {
    ...
    if (isFinancialReport) {
      ...
      title = activeReport === 'all' ? 'التقرير المالي الشامل' : (activeReport === 'delayed' ? 'تقرير المتخلفين عن الدفع' : (activeReport === 'paid' ? 'تقرير المسددين' : 'التقرير المالي للفصل'));
    } else if (isTeacherReport) {
  ...
  const renderContent = () => {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
          ...
          <div className="card" style={{ padding: 24, borderRadius: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <button onClick={() => setActiveReport('paid')} style={{ ...btnOutline, borderColor: 'var(--success-color)', color: 'var(--success-color)' }}>الطلاب المسددين (كلي أو جزئي)</button>
              <button onClick={() => setActiveReport('delayed')} style={{ ...btnOutline, borderColor: 'var(--danger-color)', color: 'var(--danger-color)' }}>المتخلفين عن الدفع (غير مسدد)</button>
            </div>
          </div>
    ...
    return (
      <div className="card" style={{ borderRadius: 12, padding: 28, position: 'relative' }}>
        <div style={{ 
          ...
          <h2 style={{ margin: 0, color: 'var(--primary-color)' }}>
            ...
            {activeReport === 'paid' && 'تقرير المسددين (كلي وجزئي)'}
            {activeReport === 'delayed' && 'تقرير المتخلفين عن الدفع'}
        ...
        <div style={{ overflowX: 'auto', border: '1px solid var(--border-color)', borderRadius: 8 }}>
          <table className="table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: 14 }}>
            <thead style={{ position: 'sticky', top: 70, zIndex: 9, background: 'var(--bg-secondary)' }}>
              {isFinancialReport ? (
                <tr>
                ...
                dataFilteredStudents.map((s, idx) => (
                  <tr key={s.id} style={{ backgroundColor: idx % 2 === 0 ? 'var(--bg-secondary)' : 'var(--bg-card)' }}>
                    ...
                    <td style={{ ...tdStyle, fontSize: 12 }}>
                      ...
                      {activeReport !== 'medical' && s.notes && <div>{s.notes}</div>}
                    </td>
                  </tr>
                ))
              ) : (
