...
export default function WithdrawnStudents({ onBack }: { onBack: () => void }) {
  ...
  const styles = {
    ...
    searchInput: { padding: '10px 16px', border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px', fontFamily: 'Cairo, sans-serif', width: '280px', outline: 'none' },
    ...
    modal: { backgroundColor: 'var(--bg-card)', borderRadius: '16px', padding: '28px 32px', width: '640px', maxWidth: '97vw', maxHeight: '95vh', overflowY: 'auto' as const, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' },
  ...
  const filteredStudents = withdrawnStudents.filter(s => {
    ...
    return (
      s.name.includes(t) ||
      s.grade.includes(t) ||
      (s.classRoom || '').includes(t) ||
      s.reason.includes(t) ||
      s.withdrawalDate.includes(t)
    );
  });
              ...
              {selectFromList && (
                <div style={{ marginTop: 8, border: '1px solid #ddd', borderRadius: 8, overflow: 'hidden' }}>
                  <input
                    ...
                    style={{ width: '100%', padding: '8px 14px', border: 'none', borderBottom: '1px solid #eee', fontSize: 14, fontFamily: 'Cairo, sans-serif', boxSizing: 'border-box', outline: 'none' }}
                    autoFocus
                  />
                  <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                    {students
                      .filter(s => !studentSearchInForm.trim() || s.name.includes(studentSearchInForm) || s.grade.includes(studentSearchInForm))
                      .map(s => (
                        <div key={s.id} onClick={() => { handleSelectStudent(s.id); setStudentSearchInForm(''); }} style={{ padding: '8px 14px', cursor: 'pointer', borderBottom: '1px solid #eee', fontSize: 14 }}>
                          {s.name} - {s.grade} {s.classRoom ? `(${s.classRoom})` : ''}
                        </div>
                      ))}
                    {students.filter(s => !studentSearchInForm.trim() || s.name.includes(studentSearchInForm) || s.grade.includes(studentSearchInForm)).length === 0 && (
                      <div style={{ padding: '12px 14px', color: '#888', fontSize: 13, textAlign: 'center' }}>لا توجد نتائج</div>
                    )}
                  </div>
                </div>
              )}
