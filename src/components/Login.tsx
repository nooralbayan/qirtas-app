...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (loginType === 'staff') {
      try {
        // Always authenticate via server — supports both plain text and bcrypt passwords
        ...
        if (data.success && data.user) {
          ...
          // Fallback: try plain-text match from local users list (offline mode)
        ...
        // Offline fallback: compare plain text only
        const user = users.find(u => u.username === username && u.password === password);
        if (user) {
          setCurrentUser(user);
        } else {
