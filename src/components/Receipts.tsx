...
export default function Receipts({ onBack }: { onBack: () => void }) {
  ...
  const [form, setForm] = useState({ studentName: '', studentId: 0, grade: '', paidAmount: 0, paymentMethod: 'نقدي' as Receipt['paymentMethod'], date: new Date().toISOString().split('T')[0] });
  ...
  const sendReceiptWhatsApp = async (receipt: Receipt) => {
    ...
    const targetPhone = student?.whatsappPhone || student?.fatherPhone;
    if (!student || !targetPhone) {
                ...
                <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
                  ...
                  <td style={{ padding: 12 }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => setEditingReceipt(r)} style={{ backgroundColor: '#f59e0b', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}>✏️ تعديل</button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {form.studentName && (
                  ...
                  <div style={{ marginBottom: 15 }}>
                    <label style={{ display: 'block', marginBottom: 5, fontWeight: 'bold' }}>تاريخ السداد</label>
                    <input 
                      type="date"
                      value={form.date || ''} 
                      onChange={e => setForm({ ...form, date: e.target.value })}
                      style={{ width: '100%', padding: '10px', borderRadius: 5, border: '1px solid #ccc', fontFamily: 'inherit' }}
                    />
                  </div>
                  <div>
                  <label style={{ display: 'block', marginBottom: 8, fontWeight: 'bold' }}>المبلغ المدفوع (د.ل)</label>
                  <input type="number" placeholder="أدخل المبلغ" value={form.paidAmount || ''} onChange={e => setForm({...form, paidAmount: Number(e.target.value)})} onWheel={e => (e.target as HTMLInputElement).blur()} style={inputStyle} />
                </div>
            ...
            <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 500, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input 
                  ...
                  style={{ width: '100%', padding: '12px 16px', fontSize: 16, borderRadius: 8, border: '1px solid #cbd5e1', outline: 'none', fontFamily: 'inherit' }}
                />
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input 
                  ...
                  style={{ width: '100%', padding: '12px 16px', fontSize: 18, borderRadius: 8, border: '1px solid #cbd5e1', outline: 'none' }}
                />
              </div>
              ...
              <div style={{ marginBottom: 30 }}>
                ...
                <select 
                  ...
                  style={{ width: '100%', padding: '12px 16px', fontSize: 16, borderRadius: 8, border: '1px solid #cbd5e1', outline: 'none' }}
                >
