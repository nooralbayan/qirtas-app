...
import { Search, ChevronLeft, Banknote, Calendar, User, Plus, Share2, FileText } from 'lucide-react-native';
...
export default function ReceiptsScreen({ navigation }: any) {
  ...
  const totalCollected = allReceipts.reduce((acc: number, r: any) => acc + (Number(r.paidAmount) || Number(r.amount) || 0), 0);
  
  const filteredReceipts = allReceipts.filter((r: any) => 
    (r.studentName && r.studentName.includes(search)) || 
    (r.receiptNumber && r.receiptNumber.toString().includes(search))
  );
        ...
        <LinearGradient colors={['#ffffff', '#f8fafc']} style={styles.card}>
          <View style={styles.cardHeader}>
            <View style={styles.amountContainer}>
              <Text style={styles.amountText}>{item.paidAmount || item.amount} <Text style={{fontSize: 14}}>د.ل</Text></Text>
            </View>
            <View style={styles.badge}>
              <Text style={styles.badgeText}>سند #{item.receiptNumber || item.id}</Text>
            </View>
          </View>
          ...
          <View style={styles.receiptDetails}>
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailText}>{item.studentName || 'غير محدد'}</Text>
            </View>
            <View style={styles.detailItem}>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View>
            ...
            <Text style={styles.subtitle}>إجمالي الإيرادات: {totalCollected} د.ل</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.searchSection}>
        <View style={styles.searchContainer}>
          <TextInput 
            ...
            placeholder="ابحث باسم الطالب أو رقم السند..."
      ...
      {filteredReceipts.length === 0 ? (
        <View style={styles.emptyState}>
          <Banknote color="#cbd5e1" size={64} style={{ marginBottom: 16 }} />
          <Text style={styles.emptyStateText}>لا توجد سندات مسجلة</Text>
        </View>
      ) : (
        <Animated.FlatList
...
const styles = StyleSheet.create({
  ...
  amountContainer: {},
  amountText: { fontSize: 24, fontWeight: '900', color: '#10b981' },
  ...
  reasonText: { color: '#64748b', fontSize: 14, textAlign: 'right', lineHeight: 22, backgroundColor: '#f8fafc', padding: 12, borderRadius: 12, borderWidth: 1, borderColor: '#f1f5f9' },
  ...
  fab: { position: 'absolute', bottom: 24, left: 24, width: 64, height: 64, borderRadius: 32, shadowColor: '#10b981', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 16, elevation: 8 },
