...
import { ChevronLeft, Book, BookOpen, GraduationCap } from 'lucide-react-native';
...
export default function SubjectsScreen({ navigation }: any) {
  ...
  const gradeSubjects = state.gradeSubjects || {};
  const subjectsList = Object.keys(gradeSubjects).map((grade) => ({
    grade,
        ...
        <LinearGradient colors={['#ffffff', '#f8fafc']} style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <View style={styles.info}>
              ...
              <Text style={styles.subtitleText}>{item.subjects.length} مواد دراسية</Text>
            </View>
          </View>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View>
            ...
            <Text style={styles.subtitle}>{subjectsList.length} صف دراسي</Text>
          </View>
        </View>
      </View>
      ...
      {subjectsList.length === 0 ? (
        <View style={styles.emptyState}>
          <BookOpen color="#cbd5e1" size={64} style={{ marginBottom: 16 }} />
          <Text style={styles.emptyStateText}>لا توجد مواد مسجلة</Text>
        </View>
      ) : (
        <Animated.FlatList
          data={subjectsList}
...
const styles = StyleSheet.create({
  ...
  subjectsList: { flexDirection: 'row-reverse', flexWrap: 'wrap', justifyContent: 'flex-start', gap: 10 },
  subjectItem: { flexDirection: 'row-reverse', alignItems: 'center', backgroundColor: '#fef3c7', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, gap: 6 },
  subjectText: { color: '#d97706', fontSize: 14, fontWeight: 'bold' },
