...
import backupRoutes, { createBackup } from './routes/backup.js';
...
// Automated Hourly Safe Backup Schedule
setInterval(async () => {
  try {
    console.log('⏰ [AUTO BACKUP] Running scheduled hourly backup...');
    await createBackup();
  } catch (err) {
    console.error('❌ [AUTO BACKUP] Failed scheduled backup:', err.message);
  }
}, 60 * 60 * 1000);
...
async function checkAndRunDailyBackup() {
    try {
        ...
        // Only run if there is data in the database
        ...
        const lastBackupTime = lastBackupDoc ? new Date(lastBackupDoc.value) : null;
        const lastModifiedTime = lastModifiedDoc ? new Date(lastModifiedDoc.value) : null;
        ...
        if (!lastBackupTime) {
            ...
            shouldBackup = true;
        } else if (now.getTime() - lastBackupTime.getTime() >= oneDayMs) {
            // It has been 24h since last backup
            // Only back up if there are actual modifications since last backup
            if (lastModifiedTime && lastModifiedTime > lastBackupTime) {
                shouldBackup = true;
            }
        }

        if (shouldBackup) {
            console.log('[Daily Backup] Starting scheduled backup check...');
            const res = await createBackup();
            console.log(`[Daily Backup] Completed. Saved to ${res.fileName}`);
        }
    } catch (err) {
        console.error('[Daily Backup] Error checking/running backup:', err.message);
    }
}

// Check on startup (delayed by 10s) and then every 1 hour
setTimeout(() => {
